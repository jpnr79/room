...
{
    if (version_compare(GLPI_VERSION, '9.5', '>=') && version_compare(GLPI_VERSION, '12.0', '<=')) {
        return true;
    } else {
        if (method_exists('Plugin', 'messageIncompatible')) {
            echo Plugin::messageIncompatible('core', '9.5', '12.0');
        } else {
            echo 'This plugin requires GLPI >= 9.5 && <= 12.0';
        }
        return false;
    }
}
